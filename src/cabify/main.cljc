(ns cabify.main
  "Kotodama WASM entrypoint for the Cabify clean-room actor (L4) — Clojure port.
  ...
  facts (`cabify.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "cabify")
(def tier "L4")
...
(def entity-specs
  [{:entity "Driver"   :plural "drivers"   :id-prefix "cabify_dri"
   ...
   {:entity "Rating"    :plural "ratings"    :id-prefix "cabify_rat"
...
(defn emit-facts
  "EAVT facts for one record: {\"cabify.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cabify-compat" :tier tier :entities entities} 200])
...
(defn start! [] :cabify-compat/ready)
